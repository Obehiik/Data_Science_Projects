...
federal_map <- c(
  ...
  "Fed-DC" = NA, "District of Columbia" = NA,  # DC excluded from state count
...
**State Variability Assessment**: We generated state-specific trajectory plots using a faceted approach that includes all 50 U.S. states. This comprehensive visualization framework reveals within-state temporal variability in the conviction-to-exoneration interval, offering detailed insights into the jurisdictional differences that shape the exoneration process throughout the American legal system.
...
#| color: primary
list(
  value=paste0(length(unique(longitudinal_data_clean$corrected_state)), " States")
)
```
